...
target =['UV254', 'TOC', 'ATZ', 'TMP', 'CBD', '2-AB', 'BZ', 'MTL', 'AC','DEA', 'DIA']
...
def create_dataset(data, time_step=3):
    ...
    for i in range(length - time_step + 1):  # 调整为 `+ 1`，确保窗口覆盖所有数据
        # X包含所有属性列
        X.append(data.iloc[i:i + time_step, :].values)
        # y仅包含目标属性列
        y.append(data.iloc[i + time_step - 1, -11:].values)
...
for train_index, test_index in kf.split(X):
    ...
    # 反标准化预测值和真实值
    y_test_scaled = scaler.inverse_transform(np.concatenate([np.zeros((y_test.shape[0], X_test.shape[2]-y_test.shape[1])), y_test], axis=1))[:, -11:]
    y_pred_scaled = scaler.inverse_transform(np.concatenate([np.zeros((y_pred.shape[0], X_test.shape[2]-y_test.shape[1])), y_pred], axis=1))[:, -11:]
