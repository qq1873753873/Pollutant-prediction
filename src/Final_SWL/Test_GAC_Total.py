...
path=f'.\\outputs_GAC\\{model_name}\\{current_time}\\'
os.makedirs(path,exist_ok=True)
res=""

# 读取并准备数据
data = pd.read_excel('F:\\Code\\coagulant-forecast\\src\\swl_lstm\\data_GAC.xlsx')
...
origin_scaled_data_df = pd.DataFrame(origin_scaled_data, columns=origin_features)#135,13
...
feature_importances[ntu_index] = 0.001  # 将 'NTU' 的重要性设为 0.01
feature_importances[conductivity_index] = 0.002  # 将 'Conductivity' 的重要性设为 0.01
# 将特征重要性和特征名称组合在一起
...
for i, target_name in enumerate(target):
    for j in range(0,97-13,13):
        plt.figure(figsize=(10, 6))
        plt.plot(all_true[j:j+13, i], label=f'True {target_name}')
        plt.plot(all_preds[j:j+13, i], label=f'Predicted {target_name}')
        plt.title(f'{target_name} Prediction vs True Values across 5-folds')
        plt.xlabel('Time Step')
        plt.ylabel(target_name)
        plt.legend()
        plt.savefig(path + f'\\Total_{target_name}_prediction_vs_true_values_{round((j+1)/13)}.png')
