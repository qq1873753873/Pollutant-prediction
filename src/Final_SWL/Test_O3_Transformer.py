...
from keras.layers import Input, Dense, Dropout, LayerNormalization, MultiHeadAttention, Add,LSTM
...
def create_dataset(data, time_step=3):
    ...
    for i in range(length - time_step):
        ...
        # y仅包含目标属性列
        y.append(data.iloc[i + time_step, -11:].values)  
...
for train_index, test_index in kf.split(X):
    ...
    # 预测
    y_pred = model.predict(X_test)#27 11
